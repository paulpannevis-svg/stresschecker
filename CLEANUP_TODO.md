...
- [ ] **HLM chart-kopie bevat recursie-bug (06-06-2026)**: `templates/hlm/consumer_metingen.html` is een kopie van de Chart.js-opbouw uit `pro/eigen_metingen.html` en bevat nog de oude baseline-constructie (post-constructie `datasets.push()` + `riChart.update()`, r210) die op `pro/eigen_metingen.html` de `RangeError: Maximum call stack size exceeded` gaf (gefixt 06-06 door baseline inline bij constructie mee te geven). Bewust **niet** mee-gepatcht: dormant spoor, wijzigen zonder test-context vermijden (afspraak do 04-06). Bij HLM-activering **niet** de kopie patchen maar consolideren naar de canonieke chart-opbouw.

- [ ] **Groen-tint-divergentie tussen RI-kleurbronnen (07-06-2026)** *(LOW PRIORITY, UI)*: na het gelijktrekken van `getColor()` op de zone-grenzen (2/4/6/8) klopt de zone overal, maar de exacte **tint** per oppervlak verschilt nog: `hrv.js getColor` "In balans" = `#2ecc71`, de kwadrant-gauge (`ZC`) = `#82d228`, de `results.html`-grafiek/tabel = `#9CCC65`. Unificeren naar één zone-palet (bv. de gauge-`ZC` als canoniek) bij een rustige UI-sessie. Alleen kleuren, geen grenzen/logica.
