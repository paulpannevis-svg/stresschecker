...
#        → Spoor 3 portal (C) → cleanup (via trap, altijd).
...
C_LOG=$(mktemp)
D_LOG=$(mktemp)
trap 'echo; echo "=== cleanup (trap) ==="; python3 /opt/stresschecker/tests/lib/cleanup.py cleanup; rm -f "'"$A_LOG"'" "'"$B_LOG"'" "'"$C_LOG"'" "'"$D_LOG"'"' EXIT
...
TOTAL_PASSED=$(( A_PASSED + B_PASSED + C_PASSED + D_PASSED ))
TOTAL_FAILED=$(( A_FAILED + B_FAILED + C_FAILED + D_FAILED ))
...
if [ "$TOTAL_FAILED" -ne 0 ] || [ "$A_RC" -ne 0 ] || [ "$B_RC" -ne 0 ] || [ "$C_RC" -ne 0 ] || [ "$D_RC" -ne 0 ]; then
    exit 1
fi
exit 0
